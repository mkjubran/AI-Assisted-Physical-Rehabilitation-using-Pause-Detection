...
resultsFile='results_OpenPose_PyTorch_2D_RF.txt'
...
if __name__ == "__main__":
   ...
   for cnt in range(npzOptions.shape[0]):
      ...
      for label in np.arange(10):
          ...
          tmp = np.load(f"../PDSaveData/SavedData_OpenPose_PyTorch_2D_E{label}_l{npzOptions[cnt,0]}_s{npzOptions[cnt,1]}_a{npzOptions[cnt,2]}.npy",allow_pickle=True)
          if label == 0:
              Zload = tmp.copy()
          else:
