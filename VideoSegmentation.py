...
mp4_source_folder_path = '../Dataset_CVDLPT_Videos_Blurred'

# Specify the folder path to output mp4 segments
mp4_destination_folder_path = '../Dataset_CVDLPT_Videos_Blurred_Segments_Corrected'

# Specify the folder path containing delination files
delination_source_folder_path = '../Dataset_CVDLPT_Videos_Blurred_SegmentsDelineation_WError_Updated'
...
for filename in os.listdir(delination_source_folder_path):
    if filename.endswith('.txt'):  # Check if the file is a text file
        ...
        mp4_source_file_path = os.path.join(mp4_source_folder_path,f"{filename.split('_3D')[0]}.mp4")
        ...
        if os.path.exists(delination_source_file_path):
             ...
             if os.path.exists(mp4_source_file_path):
                   ...
                   for cnti in range(len(Fnums)-1):
                       mp4_destination_file_path = os.path.join(mp4_destination_folder_path,f"{filename.split('_3D')[0]}_seg{cnti}.mp4")
